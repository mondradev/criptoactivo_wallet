...
const router = Router()
    ...
    .get(URL_BASE + ":network/((?!chaininfo).)*", async (req: Request, res: Response, next: NextFunction) => {
        if ((await net.getStatus()) < NetworkStatus.SYNCHRONIZED)
    ...
    .get(URL_BASE + ":network/broadcast/:hex", async (req: Request, res: Response, next: NextFunction) => {
        const raw = req.params.hex
        const network = req.params.network
